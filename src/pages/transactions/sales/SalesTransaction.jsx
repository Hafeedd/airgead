...
const SalesTransaction = () => {
    ...
    const {getSales,postSales,putSales,getAllUserAc
        ,getCodeWithBillType} = useSalesServices()
    ...
    useEffect(()=>{
        ...
        let total_cgst_amnt = tableItemList?.reduce((a,b)=>{
            return b.total && b.value?
            parseFloat(a)+parseFloat((b.total-b.value)/2):0
        },0)
        ...
        let total_scGst = tableItemList?.reduce((a,b)=>{
            return b.sgst ?
            parseFloat(a)+(parseFloat(b.sgst)*parseFloat(b.value)/100) : 0
        },0)
    ...
    const handleChange = (e) =>{
        if(e.target.type === "checkbox"){
            setSalesAdd(data=>({...data,[e.target.name]:!salesAdd[e.target.name]}))
        }
        else
        if(e.target.value === "")
            setSalesAdd(data=>({...data,[e.target.name]:null}))
        else
            setSalesAdd(data=>({...data,[e.target.name]:e.target.value}))
    }
        ...
        <div className='item_add'>
            ...
            <form ref={formRef} className='item_add_cont px-3 pb-1 pt-0'>
                <div className='row mx-0 mb-0'>
                    ...
                    {pageHeadItem==1?<SalesInvoiceDetails 
                    {...{currentEditBillType, setCurrentEditBillType,
                    billType, setBillType,getOfInvoiceData,
                    billTypeDocNo, setBillTypeDocNo,getCodeWithBillType,
                    codeWithBillTypeList, setCodeWithBillTypeList,
                    code,setCode,salesAdd,docNoRecheck,
                    setDocNoRecheck,setSalesAdd,handleChange,edit}}/>
                    :pageHeadItem==3?<SalesDeliveryDetails 
                    {...{salesAdd,setSalesAdd,handleChange}}/>
                    :pageHeadItem==2?<SalesCustomerDetails 
                    {...{customerList, setCustomerList,getAllUserAc,
                    billType, setBillType,
                    salesAdd, setSalesAdd, getCustomer}}/>
                    :pageHeadItem==4?<SalesInvoiceDetails />
                    :pageHeadItem==5?<SalesPrintingDetails />:<div></div>
                    }
                </div>
                <SalesTable
                    {...{
                salesBatchShow,setSalesBatchShow,
                tableItem,setSalesItemModal,salesAdd,
                salesList,setSalesList,edit,setEdit,
                handleSalesAllReset,setTableItemList,
                setTableItem,tableEdit,setTableEdit,
                tableItemList,cstm_id,setCstm_id,
                handleTableItemReset,getData,
                tableItemBatchList, setTableItemBatchList,
                tableItemKeys, setTableItemKeys,
                    }}
                />
                <SalesDetailFooter {...{
                    handleKeyDown,salesAdd, 
                    setSalesAdd,handleChange,edit,
                    handleSubmit,handleSalesAllReset}}/>
            </form>
            <Modal
                show={salesItemModal}
                size='lg'
            ...
            >
                <PurchaseEditList purchaseList={salesList}
                from="sales"
