...
export const StockJournal = () => {
  ...
  const {postStockJ,getStockJ,putStockJ,deleteStockJ} = useStockJournalServices()
...
const calcTotalVal = (data) =>{
    ...
    try{
      ...
      if(response2.success){
        ...
        response2.data.map(item=>{
            let a = {value:item.code,text:item.name,name:item.name,
              description:item.code,unit:item.fk_unit}
            tempList.push(a)
          })
      ...
      if(response4.success){
        ...
        response4.data.map(item=>{
          ...
          if(tempList?.length<1){
          ...
          }else if(tempList.findIndex(x=>{return x.code == item.journal_master.document_number}) == -1){
            tempList.push({
              id:item.journal_master.id,
              code:item?.journal_master?.document_number,
              narration:item.journal_master.narration,
            ...
            tempList.map((data)=>{
              if(data.code == item?.journal_master?.document_number){
                data.total_items = parseInt(data.total_items)+parseInt(item.count)
                data?.items.push({
                  name:item.item_add.name,
                  code:item.item_add.code,
                  qty:item.count,
  ...
  const handleSubmit = async (e) =>{
    e.preventDefault()
    try{
      let response, submitData = {...stockJAdd,item:stockTableItemList}
      if(!edit){
      ...
      if(response.success){
        Swal.fire(`Stock journal ${edit?"edited":"saved"} successfully`,'','success')
        setEdit(false)
        handleClearAll()
        getData()
      }else{
      ...
      Swal.fire('Something went wrong',`Failed to ${edit?"edit":"add"} stock journal`,'error')
    }
  }
  ...
  return (
    ...
    <div className="itemList_header row mx-0">
        ...
        <div className="p-2 px-3">
          <StockJournalDetails {...{
            setShowJournalFilter,stockJAdd, setStockJAdd,edit,
            stockTableItem, setStockTableItem,handleAddToTableList,
            itemNameList,unitList,stockTableItemList,handleClearAll,
            setStockTableItemList,handleResetStockTabe,handleSubmit,
            tableEdit, setTableEdit, deleteStockJ,stockJList,setEdit}}/>
        </div>
        <Modal
        ...
        <Modal.Body className="p-0 rounded-3">
          <StockJournalEdit {...{stockJList,setShowJournalFilter,
          edit,setEdit,handleClearAll,}}/>
        </Modal.Body>
        </Modal>
    </div>
</div>
