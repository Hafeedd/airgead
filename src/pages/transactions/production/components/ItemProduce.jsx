...
const ItemProduce = (props) => {
  ...
  //   if (
  //     mappedRaw.length > 0 &&
  //     mappedRaw.filter((x) => x.item_produced_name == prodItem.item_name)
  //       .length > 0
  //   ) {
  ...
  //     qty: (temp_quantity / filteredRawMaterialList[0]?.qty) * item.initial_qty,
  //     value: Number(
  ...
  //   if (
  //     mappedByprod.length > 0 &&
  //     mappedByprod.filter((x) => x.item_produced_name == prodItem.item_name)
  //       .length > 0
  //   ) {
  ...
  //     mappedLabour.filter((x) => x.item_produced_name == prodItem.item_name)
  //       .length > 0
  //   ) {
  ...
  //     item_produced_name: filteredRawMaterialList[0]?.item_details.name,
  //     amount:
  //       (item.initial_amount * temp_quantity) / filteredRawMaterialList[0]?.qty,
  //   }));
  ...
  const handleCompleteCalculation= (prodItem, temp_quantity) => {
    ...
    if(filteredRawMaterialList?.length<1) return 0;

    prodItem = { ...prodItem, fk_unit: filteredRawMaterialList[0]?.fk_unit };

    let tempMappedRaw = [];
    if (rawItems?.length > 0) tempMappedRaw = rawItems.filter(item=>item.item_produced_name === prodItem.item_name)
    
    if (tempMappedRaw.length < 1){
      filteredRawMaterialList[0]?.raw_materials?.length > 0 &&
        filteredRawMaterialList[0]?.raw_materials?.map((data) => {
          tempMappedRaw.push({
            initial_qty: data?.qty,
    ...
    let r_sum = tempMappedRaw.reduce((a, b) => a + +b.value || 0, 0);
    let rawMatWithoutCurrProd = []
    rawMatWithoutCurrProd = rawItems.filter(item=>item.item_produced_name !== prodItem.item_name) 
    setRawItems([...rawMatWithoutCurrProd,...tempMappedRaw]);

    let tempMappedByprod = [];
    if (byProductItems?.length > 0) tempMappedByprod = byProductItems.filter(item=>item.item_produced_name === prodItem.item_name)

    if (tempMappedByprod.length < 1){
      filteredRawMaterialList[0]?.by_products?.length > 0 &&
        filteredRawMaterialList[0]?.by_products?.map((data) => {
          tempMappedByprod.push({
            qty: data?.qty,
    ...
    let ByProdWithoutCurrProd = []
    ByProdWithoutCurrProd = byProductItems.filter(item=>item.item_produced_name !== prodItem.item_name) 
    setByProductItems([...ByProdWithoutCurrProd,...tempMappedByprod]);

   
    let tempMappedLabour = [];
    if (labourDetails?.length > 0) tempMappedLabour = labourDetails.filter(item=>item.item_produced_name === prodItem.item_name);

    if (tempMappedLabour.length < 1) {
      filteredRawMaterialList[0]?.expense_accounts?.length > 0 &&
        filteredRawMaterialList[0]?.expense_accounts?.map((data) => {
          // console.log(data)
          tempMappedLabour.push({
    ...
    let l_sum = tempMappedLabour.reduce((a, b) => a + +b.amount || 0, 0);
    let labourWithoutCurrProd = []
    labourWithoutCurrProd = labourDetails.filter(item=>item.item_produced_name !== prodItem.item_name) 
    setLabourDetails([...labourWithoutCurrProd,...tempMappedLabour]);
  ...
  const handleDropdownChangeItem = (e, data, prodItem, index) => {
    if (fullProdData.filter((x) => x.fk_item == data.value).length > 0)
      return 0;
    if (data.value === "") {
  ...
  const handleDropdownChangeType = (e, data, prodItem, index) => {
    if (data.value == "") {
      ...
      let tempProlist=items.filter(item => item.value === prodItem.fk_item)
      setProList([...proList,...tempProlist])
  ...
  return (
    ...
    >
      <div className='w-100 rounded-top' style={{height:'10px' ,backgroundColor:'#1F52AF'}}></div>
      <table className="w-100 ProdTable">
        <thead>
          <tr className="text-light">
            <th width="30%">Item Produced</th>
            <th width="15%">P-Type</th>
            <th width="5%">Qty</th>
            <th width="5%">Unit</th>
            <th width="5%">Cost</th>
            <th width="5%">Value</th>
          ...
          {fullProdData?.length > 0 &&
            fullProdData.map((data, key) => {
              const handleDelete = () => {
                ...
                let tempRawList = rawItems.filter(
                  (item) => item.item_produced_name != data.item_name
                );
                setRawItems([...tempRawList]);
                let tempByList = byProductItems.filter(
                  (item) => item.item_produced_name != data.item_name
                );
                setByProductItems([...tempByList]);
                let tempLabourList = labourDetails.filter(
                  (item) => item.item_produced_name != data.item_name
                );
